...
- `git` on `PATH`
- The Cursor **`agent` CLI** on `PATH` (`agent --version`) for live runs
- A [Cursor API key](https://cursor.com/dashboard?tab=integrations) only for live LLM runs (not for `./gradlew check` or `SDLC_OFFLINE=true`)
...
| Variable | Role |
| --- | --- |
| `CURSOR_API_KEY` | Required for live `bootRun`, `llmTest`, `scripts/demo.sh` |
| `SDLC_CURSOR_CLI` | Cursor CLI binary (default `agent`) |
| `SDLC_LLM_TIMEOUT` | Per Cursor CLI call (default `PT3M`). Raise to `PT5M` if you see a timeout |
| `MODEL_FAST` | Product Owner, QA, Stakeholder (default `composer-2.5`) |
| `MODEL_STRONG` | Tech Lead, Developer, PR Reviewer |
| `SDLC_HOME` | Catalog/workspace/runs root (default `.`) |
| `SDLC_PORT` | HTTP port (default `8096`) |
| `SDLC_OFFLINE` | `true` uses a canned `ChatModel` (no API key). Does not implement the demo feature. |

Export the file into the shell, or rely on your process manager. Spring reads `CURSOR_*` and `MODEL_*` from the environment.
...
Dashboard: `http://localhost:8096/` (Feature, Teams, Projects, Results).
...
Compose reads `.env` from this directory. Live runs work here too: the runtime image installs the Linux Cursor CLI at `/usr/local/bin/agent`, and Compose passes `CURSOR_API_KEY` through.

```bash
export $(grep -v '^#' .env | xargs)   # CURSOR_API_KEY, SDLC_OFFLINE=false
docker compose up --build
```

Two gotchas:

- `SDLC_CURSOR_CLI` is pinned to `agent` in `docker-compose.yml`. An absolute host path such as `/Users/you/.local/bin/agent` does not exist in the container and fails with `error: 2 (No such file or directory)`.
- The image build downloads the CLI from `cursor.com`. For an offline-only image, build with `INSTALL_CURSOR_CLI=false`.
...
`SDLC_HOME` inside the container is `/data`. Port: `http://localhost:8096/` (`SDLC_PORT` on the host).
...
Open `http://localhost:8096/`. The **Samples** tab is a numbered walkthrough, not a grid of similar cards.
...
Offline (`SDLC_OFFLINE=true`) walks the pipeline with canned JSON and **does not implement** the feature. Live Cursor CLI is required to change seed code.
...
```bash
curl -sS -X POST http://localhost:8096/api/v1/runs \
...
```bash
curl -sS http://localhost:8096/api/v1/runs/<runId>
```

SSE timeline:

```bash
curl -N http://localhost:8096/api/v1/runs/<runId>/events
```

Human stakeholder (only when the team YAML has `stakeholderMode: HUMAN`):

```bash
curl -sS -X POST http://localhost:8096/api/v1/runs/<runId>/approve \
  -H 'Content-Type: application/json' \
...
| `OutputParsingException: Failed to parse null` | Reasoning models left `content` empty. Rebuild the app (thinking JSON is recovered). Or lower `MODEL_STRONG` reasoning, or raise `CURSOR_MAX_TOKENS` |
