...
class PipelineFailuresTest {
    ...
    void userMessage_whenThereIsNoNamedAgent_keepsTheRootDetail() {
        String message = PipelineFailures.userMessage(new IllegalStateException("CURSOR_API_KEY is not set"));

        assertThat(message).contains("Pipeline failed: CURSOR_API_KEY is not set");
    }
}
