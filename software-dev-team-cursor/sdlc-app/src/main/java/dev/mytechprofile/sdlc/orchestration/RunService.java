...
public final class RunService {
     ...
     * @param properties Cursor CLI key check
     */
    public RunService(
    ...
    public RunOutcome start(RunCommand command) {
        if (!properties.offline()
                && (properties.cursorApiKey() == null
                        || properties.cursorApiKey().isBlank())) {
            throw new CatalogException(
                    "CURSOR_API_KEY is not set. Copy .env.example to .env and add a key from https://cursor.com/dashboard?tab=integrations, or set SDLC_OFFLINE=true for a canned walkthrough");
        }
