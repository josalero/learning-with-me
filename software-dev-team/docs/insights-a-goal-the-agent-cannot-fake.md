...
![The operator posts a run to sdlc-app, which reads team and project YAML, writes to a workspace git repo and to run artifacts, calls OpenRouter chat models, and runs an allowlisted build and test inside the workspace.](diagrams/system-context.png)
